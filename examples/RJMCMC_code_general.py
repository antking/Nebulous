...
usage = "python RJMCMC_code_general.py --fit True --fileName fileName -object objectName")
...
parser.add_argument("-n","--fileName",dest="fileName",default="ExampleData.txt",help="File name with line flux values that you want to fit. Format is 3 rows, N columns (where n is the number of line flux measurements), tab-separated. The first row should state the Cloudy type line names (e.g. H  1  4861.36 for H beta), the second row is line flux values, and third row is the associated errors. Can add optional 1215 continuum value using header text 'Cont 1215'. ")
parser.add_argument("-o","--object",dest="object",default="SBS1116+583A",help="Name of object whose lines are being fit.")
...
if Fit:
    ...
    # Save fit to file
    f_myfile = open(args.object+sim+'RJMCMC.pickle', 'wb')
    pickle.dump(saveobject, f_myfile)
    f_myfile.close()

else:
    # Read fit from file
    f_myfile = open(args.object+sim+'RJMCMC.pickle', 'rb')
...
for ii in range(1,6):
    ...
    x,y = zip(*shapeselect)
    plt.plot(x,y)
plt.savefig(args.object+sim+"polygon.eps", format='eps', dpi=1000)
plt.close()
plt.plot(RJMCMCobject.probarr)
plt.savefig(args.object+sim+"prob.eps", format='eps', dpi=1000)
plt.close()
plt.plot(RJMCMCobject.likearr)
plt.savefig(args.object+sim+"like.eps", format='eps', dpi=1000)
plt.close()
if ndim == 2:
    incl, cf =zip(*RJMCMCobject.thetasave)
    plt.plot(cf)
    plt.savefig(args.object+sim+"CF.eps", format='eps', dpi=1000)
    plt.close()
else:
    incl =RJMCMCobject.thetasave
plt.plot(incl)
plt.savefig(args.object+sim+"incl.eps", format='eps', dpi=1000)
plt.close()
...
plt.savefig(args.object+sim+"CoM.eps", format='eps', dpi=1000)
plt.close()
...
plt.savefig(args.object+sim+"CoM_xy.eps", format='eps', dpi=1000)
plt.close()
...
plt.hist(modelsize)
plt.savefig(args.object+sim+"modelsize.eps", format='eps', dpi=1000)
plt.close()
...
for jj in range(len(ratio_data)):
    ...
    plt.axhline(y=ratio_data[jj]-ratio_err_data[jj],color='black',linestyle="--")
    plt.savefig(sim+ratio_label[jj]+"ratio_dist.eps", format='eps', dpi=1000)
    plt.close()
...
plt.savefig(args.object+sim+"polygon_evolve.eps", format='eps', dpi=1000)
plt.close()
...
plt.ylabel(r"$\Phi_H$")
plt.xlabel(r"$n_H$")
plt.savefig(args.object+sim+"polygon_contour.eps", format='eps', dpi=1000)
plt.close()
